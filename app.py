...
<head>
    ...
    <style>
        ...
        /* --- Sidebar (Active Cloud) --- */
        .sidebar { width: 300px; background: #0a0a0a; border-left: 2px solid var(--gold); display: flex; flex-direction: column; z-index: 100; }
        ...
        .control-panel { width: 100%; max-width: 800px; background: #111; padding: 15px; border-radius: 12px; border: 1px solid #333; margin-bottom: 20px; }
        ...
        /* --- PDF Print Mode (The Fix) --- */
        @media print {
            body { background: white; color: black; height: auto; overflow: visible; }
            .sidebar, .control-panel, .card-tools, .sidebar-header { display: none !important; }
            .main { padding: 0; width: 100%; display: block; }
            .output-card { border: 1px solid #ccc; border-right: 5px solid black; break-inside: avoid; box-shadow: none; margin-bottom: 15px; color: black; }
            .report-text { color: black; font-size: 12pt; }
...
<body>
    <div class="sidebar">
        <div class="sidebar-header">الذاكرة الإمبراطورية</div>
        <div class="memory-list" id="memList"></div>
    </div>

    <div class="main">
        <h1 style="color:var(--gold); margin-bottom:20px;">STRATEGIC-AI-CORE</h1>
    ...
    <script>
        let memory = JSON.parse(localStorage.getItem('imperial_mem_v7') || '[]');

        function saveToCloud(title, content) {
            const id = Date.now();
            memory.unshift({ id, title, content, date: new Date().toLocaleTimeString() });
            localStorage.setItem('imperial_mem_v7', JSON.stringify(memory));
            renderSidebar();
        }
        ...
        function deleteItem(id, event) {
            event.stopPropagation(); // منع فتح الملف عند الحذف
            if(!confirm("هل أنت متأكد من حذف هذا السجل الاستراتيجي؟")) return;
        ...
        function loadFromCloud(id) {
            ...
            if(item) renderResults(item.content);
        }
        ...
        async function executeCore() {
            ...
            area.innerHTML = `<div style="text-align:center; color:var(--gold);">جاري الاتصال بالنواة المركزية وتحليل البيانات بلغة ${lang}...</div>`;
            ...
            try {
                ...
                area.innerHTML = "فشل النظام. تأكد من الرابط.";
            }
        }

        function renderResults(text) {
            const area = document.getElementById('resultsArea');
            const segments = text.split(/(?=\\d\\.)/); // تجزئة ذكية
            area.innerHTML = segments.map((seg, i) => `
                <div class="output-card">
                    <div class="card-tools">
                        <button class="copy-btn" onclick="copyText(this)">نسخ الجزء</button>
                    </div>
...
def chat():
    ...
    # برومبت الإمبراطور للموديل
    sys_prompt = f"""
    You are an Elite Strategy Analyst. 
    Output Language: {data.get('lang')} ONLY.
    Task: Analyze the user's input (URL or text).
    Format: Use numbered lists (1. , 2. ) for clear segmentation.
    Tone: M&A Professional (Whale Language).
    """
    ...
    try:
        ...
        return jsonify({"res": "Core Busy or Network Error."})
