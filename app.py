...
<head>
    ...
    <style>
        ...
        /* كلاس للإخفاء */
        .sidebar.collapsed {
            margin-right: -300px; /* إزاحة للخارج في وضع RTL */
            opacity: 0;
        }
        ...
        /* --- Print Mode --- */
        @media print {
            .sidebar, .control-panel, .card-tools, .toggle-btn { display: none !important; }
            .main { padding: 0; width: 100%; }
            .output-card { border: 1px solid #ccc; border-right: 5px solid black; break-inside: avoid; box-shadow: none; color: black; }
            h1 { color: black !important; text-align: center; }
        }
        
        /* Mobile adjustment */
        @media (max-width: 600px) {
            .sidebar { position: absolute; right: 0; height: 100%; box-shadow: -5px 0 10px rgba(0,0,0,0.8); }
        }
    </style>
</head>
<body>
    ...
    <div class="sidebar" id="mySidebar">
        <div class="sidebar-header">
            <span>السحابة الإمبراطورية</span>
    ...
    <script>
        ...
        function deleteItem(id, event) {
            ...
            if(!confirm("حذف هذا الملف من الذاكرة؟")) return;
        ...
        function loadFromCloud(id) {
            ...
            if(item) {
                renderResults(item.content);
                // على الجوال، نغلق القائمة تلقائياً بعد الاختيار
        ...
        async function executeCore() {
            ...
            area.innerHTML = `<div style="text-align:center; color:var(--gold);">جاري استخراج البيانات المالية وتحليل المخاطر (Whale Mode)...</div>`;
            ...
            try {
                ...
                area.innerHTML = "خطأ في الاتصال بالنواة.";
            }
        }
...
def chat():
    ...
    # --- برومبت القيمة العالية (High-Value Prompt) ---
    sys_prompt = f"""
    You are an Elite Strategy Analyst for High-Stakes M&A ($50M+ Deals).
    Output Language: {data.get('lang')} ONLY.
    
    CRITICAL TASK: Analyze the input (URL/Text) to extract:
    1. Financial & Technical Valuation: Look for ANY numbers, revenue, growth % (CAGR), or code efficiency metrics.
    2. Cyber-Due Diligence: Identify hidden risks, Shadow AI usage, or scalability bottlenecks.
    3. Game Theory & Scenarios: Predict future market moves for the acquirer.
    
    Format: Use numbered lists (1. , 2. ) with professional 'Whale' language.
    """
